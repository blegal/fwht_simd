...
inline bool fix_xor_list(uint16_t *list1, const uint16_t *list2, const symbols_i32<gf_size> *internal, const int N)
{
	...
	int32_t best_1err_score = 0; // 32 max
	for (int i = 0; i < N; i += 1)
	{
		const int new_symb = total_xor ^ list1[i];
		const int32_t new_proba = internal[i].value[new_symb];
	...
	int pos_argmax_2 = 0;
	int32_t val_argmax_2 = 0;
	for (int i = 0; i < N; i += 1)
	{
		const int n_symbo = list2[i];
		const int32_t n_proba = internal[i].value[n_symbo];
	...
	const int32_t local_proba = internal[pos_argmax_2].value[list2[pos_argmax_2]]; // proba2[pos_argmax_2];

	int best_2err_position = 0;	 // 32 max
	int best_2err_symbole = 0;	 // 32 max
	int32_t best_2err_score = 0; // 32 max
	for (int i = 0; i < N; i++)
	{
		if (i == pos_argmax_2)
			continue;
		const int new_symb = local_xor ^ list1[i];
		const int32_t new_proba = local_proba * internal[i].value[new_symb];
