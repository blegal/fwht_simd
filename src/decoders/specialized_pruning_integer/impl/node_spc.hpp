...
#include "decoders/specialized_pruning_integer/fix_xor_list.hpp"
...
//
//
inline void remove_xors(uint16_t *values, int size)
{
	if (size == 1)
	{
		return;
...
void decoder_specialized_pruning_integer<gf_size>::middle_node_pruned_spc_after_f(
	...
	const int symbol_id)					 // Symbol ID is the index of the FIRST symbol in the symbols array
{
	...
	{
		arg_2[j] = f_argmax2<gf_size>(inputs[j].value, arg_1[j]);
	}
	//
...
//
template <int gf_size>
void decoder_specialized_pruning_integer<gf_size>::middle_node_pruned_spc_after_g(
	symbols_i32<gf_size> *__restrict inputs, // Inputs are the symbols from the channel (from the right)
	uint16_t *__restrict decoded,			 // Decoded symbols are the final output of the decoder (done on the left)
	uint16_t *__restrict symbols,			 // Symbols are the ones going from leafs to root (done on the left)
	int size,								 // Size is the number of symbols (should be a power of 2)
