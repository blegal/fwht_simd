	...
	const int n_symbols)
{
	for (int s = 0; s < n_symbols; s++)
	{
		...
		LZC_normalize<gf_size, I_type::NBITS, BlockType::F>(dst[s].value);
	}
}
//
//
	...
	const int n_symbols)
{
	for (int s = 0; s < n_symbols; s++)
	{
		...
		LZC_normalize<gf_size, I_type::NBITS, BlockType::PF>(tmp_a.value);
		...
		LZC_normalize<gf_size, I_type::NBITS, BlockType::PF>(tmp_b.value);
#if FWHT_COUNTER_ENABLE
		fwht_call_counter += 1;
#endif
