...
void decoder_specialized_pruning<gf_size>::execute(void* s_channel, uint16_t * decoded)
{
    ...
    f_function_proba_in<gf_size>(internal, channel, channel + n, n);
    //
    ...
    //
    g_function_proba_in<gf_size>(internal,  channel, channel + n, symbols, n);
    //
    ...
    const int symbol_id)    // Symbol ID is the index of the FIRST symbol in the symbols array
{
    ...
    }else if( left_edge == SPC_FROM_F)         { middle_node_pruned_spc_after_f         (internal, decoded, symbols, n, symbol_id                   );
#if 0
...
//  printf("-> middle_node_pruned_after_g (size = %d, f_tree_cnt = %d) - right = %s\n", size, f_tree_cnt, s_next_node[f_tree->next_node_status[f_tree_cnt]].c_str());
    ...
    } else if (right_edge == RATE_1_FROM_F)      { exit(EXIT_FAILURE);
    } else { printf("(EE) We should never be there (%s, %d)\n", __FILE__, __LINE__); exit(EXIT_FAILURE); }
    //
    //
    //
