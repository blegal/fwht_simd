...
template <int gf_size>
inline __attribute__((always_inline)) int position(const float * vector, float value) {
...
template <int gf_size>
float compute_entropy(const float * probs) {

    const float inv_log_q = 1.0f / logf(static_cast<float>(gf_size));
    float       entropy   = 0.0f;
    for (int i = 0; i < gf_size; ++i) {
        if (probs[i] > 0.0f) {
            entropy -= probs[i] * logf(probs[i]) * inv_log_q;
        }
    }
    return entropy;
}
