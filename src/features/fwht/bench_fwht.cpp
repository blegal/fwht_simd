...
int main(int argc, char *[]) {
    ...
    for (int size = 16; size <= 256; size *= 2) {
        ...
        for (int32_t loop = 0; loop < nTest; loop += 1) {
            if (size == 8) {
                fwht<8>(tab_e, tab_d);
                normalize<8>(tab_e, 0.35355339059f);
                fwht<8>(tab_d, tab_e);
                normalize<8>(tab_d, 0.35355339059f);
            }
            if (size == 16) {
                fwht<16>(tab_e, tab_d);
                normalize<16>(tab_e, 0.25f);
                fwht<16>(tab_d, tab_e);
                normalize<16>(tab_d, 0.25f);
            }
            if (size == 32) {
                fwht<32>(tab_e, tab_d);
                normalize<32>(tab_e, 0.17677669529f);
                fwht<32>(tab_d, tab_e);
                normalize<32>(tab_d, 0.17677669529f);
            }
            if (size == 64) {
                fwht<64>(tab_e, tab_d);
                normalize<64>(tab_e, 0.125f);
                fwht<64>(tab_d, tab_e);
                normalize<64>(tab_d, 0.125f);
            }
            if (size == 128) {
                fwht<128>(tab_e, tab_d);
                normalize<128>(tab_e, 0.08838834764f);
                fwht<128>(tab_d, tab_e);
                normalize<128>(tab_d, 0.08838834764f);
            }
            if (size == 256) {
                fwht<256>(tab_e, tab_d);
                normalize<256>(tab_e, 0.0625f);
                fwht<256>(tab_d, tab_e);
                normalize<256>(tab_d, 0.0625f);
            }
        }
        ...
        for (int32_t loop = 0; loop < nTest; loop += 1) {
            if (size == 8) {
                fwht_norm_neon<8>(tab_d);
                fwht_norm_neon<8>(tab_d);
            }
            if (size == 16) {
                fwht_norm_neon<16>(tab_d);
                fwht_norm_neon<16>(tab_d);
            }
            if (size == 32) {
                fwht_norm_neon<32>(tab_d);
                fwht_norm_neon<32>(tab_d);
            }
            if (size == 64) {
                fwht_norm_neon<64>(tab_d);
                fwht_norm_neon<64>(tab_d);
            }
            if (size == 128) {
                fwht_norm_neon<128>(tab_d);
                fwht_norm_neon<128>(tab_d);
            }
            if (size == 256) {
                fwht_norm_neon<256>(tab_d);
                fwht_norm_neon<256>(tab_d);
            }
        }
