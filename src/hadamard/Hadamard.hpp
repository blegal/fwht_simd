...
inline const float * get_Hadamard_line(const int line) {
    if constexpr (gf_size == 8) {
        return &Hadamard_8x8[line][0];
    } else if constexpr (gf_size == 16) {
        return &Hadamard_16x16[line][0];
    } else if constexpr (gf_size == 32) {
        return &Hadamard_32x32[line][0];
    } else if constexpr (gf_size == 64) {
        return &Hadamard_64x64[line][0];
    } else if constexpr (gf_size == 128) {
        return &Hadamard_128x128[line][0];
    } else if constexpr (gf_size == 256) {
        return &Hadamard_256x256[line][0];
    } else if constexpr (gf_size == 512) {
        return &Hadamard_512x512[line][0];
    } else if constexpr (gf_size == 1024) {
        return &Hadamard_1024x1024[line][0];
    } else if constexpr (gf_size == 2048) {
        return &Hadamard_2048x2048[line][0];
    } else if constexpr (gf_size == 4096) {
        return &Hadamard_4096x4096[line][0];
    } else {
        static_assert(gf_size == -1, "We never should be here !");
    }
}
